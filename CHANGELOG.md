...
- 1.21.0 (unreleased)
  - don't report underrun errors in the GUI but log them, they only seem to happen on WasApi, but are never audible because swyh-rs rebuffers anyway
...
- 1.20.2 (Apr 15 2026 dheijl)
  ...
  - run the first SSDP discovery twice in a row as the first run is sometimes incomplete on Linux
  - more refactoring in `ChannelStream`, and simplify and optimize SIMD path for LPCM/WAV/RF64
...
- 1.20 (Mar 7 2026, dheijl)
  ...
  - improve CLI command line parsing (meaningful error messages, parse all arguments even if errors)
