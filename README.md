...
- some AVTtransport renderers will stop when detecting a pause between songs, you can use the "*Autoresume*" checkbox if you encounter this problem. However, with current versions of swyh-rs you should not have to use this checkbox. Try to disable "Chunked Transfer Encoding" frst instead. It does not work well with AVTransport anyway, but it does work reliably with OpenHome renderers in my experience (if I disconnect from the renderer's side swyh-rs will reconnect automatically and reliably when Autoresume is set).
...
- Since version 1.2.2, swyh-rs will send silence to connected renderers if no sound is being captured because no audio is currently being played. This prevents some renderers from disconnecting because they have not received any sound for some time. Apparently sending silence keeps them happy. 
