...
widget-style-label = Widget style: { $style }
language-label = Language: { $lang }
warn-language-changed = Language changed to { $lang }, restart required!!
warn-widget-style-changed = Widget style changed to { $style }, restart required!!
...
chk-enable-slimproto = Enable SlimProto
...
sr-system-default = System Default ({ $rate } Hz)
...
status-starting-slimproto = Starting SlimProto discovery
status-slimproto-disabled = SlimProto discovery disabled
...
warn-slimproto-changed = SlimProto support changed, restart required!!
