...
widget-style-label = Stile widget: { $style }
language-label = Lingua: { $lang }
warn-language-changed = Lingua cambiata in { $lang }, riavvio richiesto!!
warn-widget-style-changed = Stile widget cambiato in { $style }, riavvio richiesto!!
...
chk-enable-slimproto = Abilita SlimProto
ssdp-interval-label = Intervallo SSDP (minuti)
btn-ssdp-discover = Avvia scoperta SSDP ora
log-level-label = Log: { $level }
fmt-label = Format: { $format }
chk-24bit = 24 bit
sample-rate-label = Frequenza (Hz):
sr-system-default = Predefinito di sistema ({ $rate } Hz)
http-port-label = Porta HTTP:
chk-use-dither = Dither TPDF 16 bit
...
status-starting-slimproto = Avvio rilevamento SlimProto
status-slimproto-disabled = Rilevamento SlimProto disabilitato
...
warn-slimproto-changed = Supporto SlimProto modificato, riavvio richiesto!!
