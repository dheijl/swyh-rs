                ...
                std::thread::spawn(move || {
                    ...
                    if !VALID_URLS.contains(&rq.url()) {
                        ...
                        if let Err(e) = rq.respond(response) {
                            ui_log(&format!(
                                "{} {remote_addr} [{e}]",
                                t!("http_streaming_request_terminated")
                            ));
                        }
                        return;
                    }
                    ...
                    if matches!(rq.method(), Method::Get) {
                        ...
                        ui_log(&format!(
                            "{} {streaming_format}, {} {:?}, channels=2, rate={}, {} {}",
                            t!("streaming"),
                            t!("input_sample_format"),
                            wd.sample_format,
                            wd.sample_rate.0,
                            t!("to"),
                            rq.remote_addr().unwrap()
                        ...
                        feedback_tx_c
                            ...
                            .unwrap();
                        ui_log(&format!("{} {remote_addr}", t!("streaming_ended")));
                        ...
                        if let Err(e) = rq.respond(response) {
                            ui_log(&format!(
                                "{} {remote_addr} [{e}]",
                                t!("http_head_connection_terminated")
                            ));
                        }
                        ...
                        if let Err(e) = rq.respond(response) {
                            ui_log(&format!(
                                "{} {remote_addr} [{e}]",
                                t!("http_post_connection_terminated")
                            ));
                        }
                    }
                });
